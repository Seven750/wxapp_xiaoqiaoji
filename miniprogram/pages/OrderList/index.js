const sicon = "data:image/png;base64,";
const picon = "data:image/png;base64,";
const buttons = [
  {
    label: '进货',
    icon: picon
  },
  {
    label: '售货',
    icon: sicon
  }
];
Page({

  /**
   * 页面的初始数据
   */
  data: {
    opts:{},
    total:0,
    current_page:1,
    hidden:false,
    status: true,
    msg: {
      icon: '../../../images/iconfont-order.png',
      title: '您还没有相关的订单',
      text: '可以点击右方按钮添加新的订单'
    },
    buttons,
    current:["0"],
    localfiles:[],
    inputShowed: false,
    params:{sort:"time",type:"timeasc"},
    inputVal: "",
    items: [{
      type: 'radio',
      label: '订单类别',
      value: 'orders',
      children: [{
        label: '进货',
        value: 'purchase',
      },{
        label: '售出',
        value: 'sales',
      },{
      label: '现结',
      value: 'paidorder',
      },{
      label: '赊账',
      value: 'unpaidorder',
      },{
      label: '已收账',
      value: 'complete',
      },{
      label: '全部订单',
      value: 'allorder',
      },],
      groups: ['001'],
    }, {
      type: 'sort',
      label: '时间',
      value: 'time',
      groups: ['002'],
      }, {
      type: 'sort',
      label: '金额',
      value: 'money',
      groups: ['003'],
    }],
    orders: "",
    type:"",
    current:["0"]
  },

  /**
   * 生命周期函数--监听页面加载
   */

  onLoad: function (options) {
    wx.showLoading({
      title: '正在加载数据',
    })
    const that = this
    const db = wx.cloud.database();
    db.collection("Order").count().then(res=>{
      if(res.total== 0 )
      {
        this.setData({
          status: false
        })
        wx.hideLoading();
      }
      else{
        const total = res.total;
        const page_num = parseInt(total / 15) + 1;
        let hidden = false;
        if (page_num>1)
          hidden = true
        that.setData({
          total: page_num,
          hidden: hidden
        })
        this.querytimeandmoney("createTime", "desc")
      }
    })
  },
  onchangePage:function(e){
    wx.showLoading({
      title: '正在加载数据',
    })
    this.setData({
      current_page:e.detail.current
    })
    const params = this.data.params;
    const page = parseInt(e.detail.current - 1) * 15;
    if (params.sort != "time" && params.sort !="money")
      if(e.detail.current!=1)
        this.query_skip(params,page)
      else
        this.query(params);
    else
      if(e.detail.current!=1)
      {
        if (params.type == "allorder") {
          this.querytimeandmoney_skip("createTime", "desc", page)
        } else if (params.type == "timeasc") {
          this.querytimeandmoney_skip("createTime", "desc", page)
        } else if (params.type == "timedesc") {
          this.querytimeandmoney_skip("createTime", "asc", page)
        } else if (params.type == "moneyasc") {
          this.querytimeandmoney_skip("total_amount", "desc", page)
        } else if (params.type == "moneydesc") {
          this.querytimeandmoney_skip("total_amount", "asc", page)
        }
      }
      else{
        if (params.type == "allorder") {
          this.querytimeandmoney("createTime", "desc")
        } else if (params.type == "timeasc") {
          this.querytimeandmoney("createTime", "desc")
        } else if (params.type == "timedesc") {
          this.querytimeandmoney("createTime", "asc")
        } else if (params.type == "moneyasc") {
          this.querytimeandmoney("total_amount", "desc")
        } else if (params.type == "moneydesc") {
          this.querytimeandmoney("total_amount", "asc")
        }
      }

  },
  onChangefilter:function(e){
    wx.showLoading({
      title: '正在加载数据',
    })
    const that = this;
    const { checkedItems } = e.detail
    const params = {}
    checkedItems.forEach((n) => {
      if (n.checked) {
        if (n.value === 'orders') {
          const selected = n.children.filter((n) => n.checked).map((n) => n.value).join(' ')
          params.sort = n.value
          params.type = selected
        } else if (n.value === 'time') {
          params.sort = n.value
          params.type = n.sort === 1 ? 'timeasc' : 'timedesc'
        }else if (n.value === 'money') {
          params.sort = n.value
          params.type = n.sort === 1 ? 'moneyasc' : 'moneydesc'
        }
        that.setData({
          params: params,
          current_page:1
        })
      }
    })
    this.getRepos(params)
  },
  querycount:function(obj){
    const that = this
    const db = wx.cloud.database();
    if(obj!={})
    {
      db.collection("Order").where(obj).count().then(res => {
        if (res.total == 0)
          this.setData({
            status: false
          })
        else {
          const total = res.total;
          const page_num = parseInt((total-1)/ 15) + 1;
          let hidden = false;
          if (page_num > 1)
            hidden = true
          that.setData({
            total: page_num,
            current_page:1,
            hidden: hidden
          })
        }
      })
      
    }else{
      db.collection("Order").count().then(res => {
        if (res.total == 0)
          this.setData({
            status: false
          })
        else {
          const total = res.total;
          const page_num = parseInt(total / 15) + 1;
          let hidden = false;
          if (page_num > 1)
            hidden = true
          that.setData({
            total: page_num,
            current_page: 1,
            hidden: hidden
          })
        }
      })
    }
    
  },
  query:function(params){
    const that = this;
    const db = wx.cloud.database();
    let obj = {}
    if (params.type == "purchase")
      obj = { category: "进货" }
    else if (params.type == "sales")
      obj = { category: "售出" }
    else if (params.type == "paidorder")
      obj = { paymentType: "现结" }
    else if (params.type == "unpaidorder")
      obj = { paymentType: "赊账" }
    else if (params.type == "complete")
      obj = { paymentType: "已收账" }
    db.collection("Order").where(obj).orderBy("createTime", "desc").limit(15).get().then(res =>that.resultres(res))
  },
  query_skip: function (params,sk) {
    let obj = {}
    if (params.type == "purchase") 
      obj = { category: "进货" }
    else if (params.type == "sales") 
      obj = { category: "售出" }
    else if (params.type == "paidorder") 
      obj = { paymentType: "现结" }
    else if (params.type == "unpaidorder") 
      obj = { paymentType: "赊账" }
    else if (params.type == "complete") 
       obj = { paymentType: "已收账" }
    const that = this;
    const db = wx.cloud.database();
    db.collection("Order").where(obj).orderBy("createTime", "desc").skip(sk).limit(15).get().then(res =>that.resultres(res))
  },
  querytimeandmoney: function (fieldName,order){
    const that = this;
    const db = wx.cloud.database();
    db.collection("Order").orderBy(fieldName, order).limit(15).get().then(res => that.resultres(res))
  },
  querytimeandmoney_skip: function (fieldName, order,sk) {
    const that = this;
    const db = wx.cloud.database();
    db.collection("Order").orderBy(fieldName, order).skip(sk).limit(15).get().then(res => that.resultres(res))
  },
  resultres:function(res){
    if (res.data[0].Files == "")
      this.setData({
        orders: res.data,
      },()=>wx.hideLoading())
    else {
      if (res.data[0].Files == "")
        this.setData({
          orders: res.data,
        })
      else {
        const picfiles = []
        const files = res.data[0].Files
        for (let i = 0; i < files.length; i++) {
          const pic_src = files[i].fileID
          picfiles.push(pic_src)
        }
        this.setData({
          localfiles: picfiles,
          orders: res.data
        }, () => wx.hideLoading())
      }
    }
  },
  getRepos: function (params = {}){
    if (params.type == "purchase"){
      this.querycount({ category: "进货" });
      this.query({ category:"进货"})
    } else if (params.type =="sales"){
      this.querycount({ category: "售出" });
      this.query({ category: "售出" })
    } else if (params.type == "paidorder"){
      this.querycount({ paymentType: "现结" });
      this.query({ paymentType: "现结" })
    } else if (params.type == "unpaidorder"){
      this.querycount({ paymentType: "赊账" });
      this.query({ paymentType: "赊账" })
    } else if (params.type == "complete") {
      this.querycount({ paymentType: "已收账" });
      this.query({ paymentType: "已收账" })
    } else if (params.type == "allorder"){
      this.querycount({});
      this.querytimeandmoney("createTime", "desc")
    } else if (params.type == "timeasc"){
      this.querycount({});
      this.querytimeandmoney("createTime", "desc")
    } else if (params.type == "timedesc") {
      this.querycount({});
      this.querytimeandmoney("createTime", "asc")
    } else if (params.type == "moneyasc") {
      this.querycount({});
      this.querytimeandmoney("total_amount", "desc")
    } else if (params.type == "moneydesc") {
      this.querycount({});
      this.querytimeandmoney("total_amount", "asc")
    }
  },
  onChangeaccordion:function(e){
    var current = [];
    current.push(e.detail.key)
    if(e.detail.key!=null&&e.detail.key!="")
    {
      const files = this.data.orders[parseInt(e.detail.key)].Files;
      if(files=="")
      {
        this.setData({
          current: current,
          localfiles:[]
        })
        return
      }
      const picfiles = []
      for (let i = 0; i < files.length; i++) {
        const pic_src = files[i].fileID
        picfiles.push(pic_src)
      }
      this.setData({
        localfiles:picfiles,
        current: current
      })
      return
    }else{
      this.setData({
        current: [],
        localfiles: []
      })
    }
  },
  PreviewImage: function (e) {
    wx.previewImage({
      current: e.currentTarget.id, // 当前显示图片的http链接
      urls: this.data.localfiles // 需要预览的图片http链接列表
    })
  },
  confirm:function(e){
    const index = e.currentTarget.id;
    const orderid = this.data.orders[index]._id;
    wx.navigateTo({
      url: '../../pages_order/pages/confirm_order/index?orderid='+orderid,
    })
  },
  fabonChange(e) {
    console.log('onChange', e)
  },
  fabonClick(e) {
    if (e.detail.index == 0)
      wx.navigateTo({
        url: '../../pages_order/pages/AddPurchaseOrders/index',
      })
    else if (e.detail.index == 1)
      wx.navigateTo({
        url: '../../pages_order/pages/AddSalesOrders/index?client='+"none",
      })
    else
      console.log("no page")
  }
})